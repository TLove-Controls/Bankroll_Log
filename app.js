...
function cacheState(state) {
    localStorage.setItem(STATE_CACHE_KEY, JSON.stringify(normalizeState(state)));
}
...
function normalizeBet(rawBet = {}) {
    ...
    const parlayLegs = normalizedType === 'Parlay' && (rawBet.parlayLegs === 2 || rawBet.parlayLegs === 3 || rawBet.parlayLegs === '2' || rawBet.parlayLegs === '3')
...
function onBetTypeChange() {
    const typeValue = document.getElementById('betType').value;
    syncParlayFields(typeValue);
}
...
async function loadState() {
    try {
        ...
        applyState(state);
        cacheState(state);
        setSaveStatus(hasCloudSync() ? 'Synced with cloud' : 'Loaded saved data');
    } catch (error) {
        ...
        setSaveStatus(hasCloudSync() ? 'Using cached cloud data' : 'Loaded cached data');

        if (hasCloudSync()) {
            showNotification('Cloud sync is unavailable. Showing your cached data.', 'error');
        }
    }
}

async function persistState() {
    setSaveStatus(hasCloudSync() ? 'Syncing...' : 'Saving...');
    const response = await requestJSON('/api/state', {
        method: 'PUT',
    ...
    applyState(state);
    cacheState(state);
    setSaveStatus(hasCloudSync() ? 'Synced with cloud' : 'Saved');
}
        ...
        <article class="bet-log-item">
            <div class="bet-log-main">
                ...
                <div class="bet-log-meta">
                    ...
                    <span>Return ${bet.return > 0 ? `$${bet.return.toFixed(2)}` : '-'}</span>
                    <span>Edge ${escapeHtml(bet.edge || '-')}</span>
                </div>
...
async function saveSyncSettings(event) {
    ...
    try {
        await loadState();
        updateUI();
        showNotification(settings.apiUrl ? 'Cloud sync settings saved.' : 'Using local desktop storage.');
    } catch (error) {
...
async function syncNow() {
    try {
        await loadState();
        updateUI();
        showNotification(hasCloudSync() ? 'Synced with cloud.' : 'Reloaded local data.');
    } catch (error) {
...
function clearSyncSettings() {
    ...
    loadSyncSettingsIntoForm();
    updateSyncModeStatus();
    showNotification('Cloud sync disabled on this desktop.');
}
